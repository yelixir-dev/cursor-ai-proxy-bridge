...
function renderCredentials(){
  ...
  rows.forEach(({config,state})=>{
    ...
    const plan=make('select','plan-input');plan.dataset.adminControl='';plan.disabled=locked;plan.setAttribute('aria-label',id+' 플랜');
    ...
    capabilityCell.append(switchControl(config.capabilities&&config.capabilities.fable===true,locked||config.plan!=='ultra',id+' Fable capability · Ultra 플랜 전용',async event=>{await patchConfig({credentials:[{id,capabilities:{fable:event.currentTarget.checked}}]});}));

...
describe('measured account comparability', () => {
  ...
  it.each([
    ['missing bridge credential', jwt('native'), {}, 'bridge_credential_missing'],
    [
      'opaque native token',
      'opaque',
      { CURSOR_AUTH_TOKEN: jwt('bridge') },
      'native_credential_opaque',
    ],
    [
      'malformed native token',
      'x.invalid.x',
      { CURSOR_AUTH_TOKEN: jwt('bridge') },
    ...
    [
      'opaque bridge token',
      jwt('native'),
      { CURSOR_AUTH_TOKEN: 'opaque' },
      'bridge_credential_opaque',
    ],
    [
      'malformed bridge token',
      jwt('native'),
      { CURSOR_AUTH_TOKEN: 'x.invalid.x' },
      'bridge_credential_malformed',
    ],
  ] as const)('records %s as unproved', async (_name, native, environment, reason) => {
